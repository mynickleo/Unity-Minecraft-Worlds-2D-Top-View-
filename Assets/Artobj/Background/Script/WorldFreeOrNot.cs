...
public class WorldFreeOrNot : MonoBehaviour
{
    ...
    public void FixedUpdate()
    {
        ...
        if (Directory.Exists(Firstfolder))
        {
            ...
            if (Directory.Exists(pathToOtherFile))
            {
                ...
                {
                    GetComponentInChildren<Text>().text = "Свободно";
                }
            }
            else
            {
    ...
    public void OnClick()
    {
        ...
        else
        {
            ...
            SceneManager.LoadScene("Minecraft_Worlds2D");
            Directory.CreateDirectory(pathToOtherFile);
            StreamWriter writerSeed = new StreamWriter(pathToSid, false); //Если нет, тогда он создаст мир, но значения оттуда браться не будут
