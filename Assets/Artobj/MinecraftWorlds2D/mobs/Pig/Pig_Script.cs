...
public class Pig_Script : MonoBehaviour
{
    ...
    public GameObject Pig_down;
    public GameObject Pig_up;

    public GameObject Pig;

    //some_varibale for relax position
    int variable_for_relax = 0;
    ...
    public void Update()
    {
        if (action > 0)
        {
            transform.Translate(go * 0.01f, 0, 0);
            action--;
            GetComponent<Animation>().Play("Mobs_pig_anim_left_right");
        }
        if (action == 0) variable_for_relax = 0;
        if (variable_for_relax == 0) {
            GetComponent<Animation>().Play("Pig_Relax");
            variable_for_relax = 1;
                }
    }
    ...
    public void OnCollisionEnter2D(Collision2D collision)
    {
        if (variable_start_transform == 1)
        {
            ...
            {
                if (transform.rotation.y < 0) transform.position = new Vector3(transform.position.x + 0.1f, transform.position.y);
                else transform.position = new Vector3(transform.position.x - 0.1f, transform.position.y);
                Pig.transform.position = transform.position;
            }
